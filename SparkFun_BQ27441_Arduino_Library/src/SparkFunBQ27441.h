...
public:
	...
	*/
	...
	/**
		...
		@param capacity of battery (must be between 0 and 8000)
		       energy of battery (must be between 0 and 32767)
		@return true if capacity successfully set.
	*/
	bool setCapacity(uint16_t capacity, uint16_t energy);

	/**
	    Configures the Taper Rate of battery fuel gauge.
		
		@param Taper Rate (must be between 0 and 2000)
		@return true if taper rate successfully set.
	*/
	bool setTaperRate(uint16_t taperRate);

	/**
	    Set the Terminate Voltage of the battery fuel gauge 
		
		@param voltage (must be between 2500mV and 3700mV)
	...
	*/
	...
	/**
		...
		@param status 
		  Learning Cycle: set bit 0 (0x01) and bit 1 (0x02). 
		  Seal State: set bit 7 (0x80).
		@return true if R_a Table successfully set
	*/
	bool setUpdateStatusReg(uint8_t status);

	/**
		Reads and returns the Qmax value of battery fuel gauge.
		@return Qmax value
	*/
	uint16_t Qmax(void);

	/**
	    Set the Qmax value of battery fuel gauge.
		
		@param taper rate (unsigned 16-bit value)
		@return true if taper rate successfully set.
	*/
	bool setQmax(uint16_t qmax);

	/**
		Read from the R_a RAM Table of fuel gauge into a buffer

		@param points to the R_a Table buffer to be written
		@return always true
	*/
	bool RaTable(uint16_t * ra);

	/**
		Set the R_a RAM Table of fuel gauge from a buffer

		@param points to the R_a Table buffer to be read
		@return true if R_a Table successfully set
	*/
	bool setRaTable(uint16_t * ra);
...
private:
	...
	*/	
	...
	/**
	    Issue an exit-resim to the BQ27441-G1A (No OCV measurement)
	...
	*/
	...
	/**
	    Read a specified number of bytes from extended data specifying a class ID and 
		position offset.
		...
		@return true on success
	*/
	bool readExtendedData(uint8_t classID, uint8_t offset, uint8_t * data, uint8_t len);
	
	/**
	    Write a specified number of bytes to extended data specifying a class ID and 
		position offset.

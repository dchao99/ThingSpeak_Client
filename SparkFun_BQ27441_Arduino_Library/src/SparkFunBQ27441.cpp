...
- Added setTerminateVoltage() - To set the system terminate voltage.
- Added setUpdateStatusReg() - To enter into learning cycle.
- Added Qmax() and setQmax() - To improve initial accuracy of battery fuel gauge.
- Added RaTable() and setRaTable() - To read and set the R_a RAM Table.
- Changed readExtendedData() parameters to read a block of data instead of a byte.
...
// Configures the design capacity and energy of the connected battery 
// (capacity=0~8000) (energy=0~32767).
bool BQ27441::setCapacity(uint16_t capacity, uint16_t energy)
{
	// Write to STATE subclass (82) of BQ27441 extended memory.
	// Offset 0x0A (10) and 0x0C (12)
	// capacity & energy are both 2-byte piece of data - MSB first
	uint16_t cap = constrain(capacity, 0, 8000);
	uint16_t ene = constrain(energy, 0, 32767);
	uint8_t  capMSB = cap >> 8;
	uint8_t  capLSB = cap & 0x00FF;
	uint8_t  eneMSB = ene >> 8;
	uint8_t  eneLSB = ene & 0x00FF;
	uint8_t  stateData[4] = {capMSB, capLSB, eneMSB, eneLSB};
	return writeExtendedData(BQ27441_ID_STATE, 10, stateData, 4);
}

// Configures the Taper Rate of the connected battery (0~2000).
bool BQ27441::setTaperRate(uint16_t taperRate)
{
	// Write to STATE subclass (82) of BQ27441 extended memory.
	// Offset 0x1B (27)
	// taperRate is a 2-byte piece of data - MSB first
	uint16_t rate = constrain(taperRate, 0, 2000);
	uint8_t rateMSB = rate >> 8;
	uint8_t rateLSB = rate & 0x00FF;
	uint8_t stateData[2] = {rateMSB, rateLSB};
	return writeExtendedData(BQ27441_ID_STATE, 27, stateData, 2);
}

// Set the Terminate Voltage of the connected battery (2500mV~3700mV).
bool BQ27441::setTerminateVoltage(uint16_t voltage)
{
	...
	return writeExtendedData(BQ27441_ID_STATE, 16, stateData, 2);
}

// Set the Update Status Register of the battery fuel gauge.
bool BQ27441::setUpdateStatusReg(uint8_t status)
{
	// Write to STATE subclass (82) of BQ27441 extended memory.
	// Offset 0x02 (2)
	// status is 1 byte 
	// Learning Cycle: set bit 0 (0x01) and bit 1 (0x02). Seal State: set bit 7 (0x80).
	return writeExtendedData(BQ27441_ID_STATE, 2, &status, 1);
}

// Get the Qmax value of the battery fuel gauge.
uint16_t BQ27441::Qmax(void)
{
	uint8_t qmaxData[2];
	readExtendedData(BQ27441_ID_STATE, 0, qmaxData, 2);
	return ((uint16_t)qmaxData[0] << 8) | qmaxData[1];
}

// Set the Qmax to a value between 0 and 32767
bool BQ27441::setQmax(uint16_t qmax)
{
	uint16_t q = constrain(qmax, 0, 32767);
	uint8_t qMSB = q >> 8;
	uint8_t qLSB = q & 0x00FF;
	uint8_t qmaxData[2] = {qMSB, qLSB};
	return writeExtendedData(BQ27441_ID_STATE, 0, qmaxData, 2);
}

// Read the R_a RAM Table of the battery fuel gauge.
bool BQ27441::RaTable(uint16_t * ra)
{
	uint8_t raData[30];
	readExtendedData(BQ27441_ID_R_A_RAM, 0, raData, 30);
	for (int i = 0; i < 15; i++) {
		ra[i] = ((uint16_t)raData[i*2] << 8) | raData[i*2+1];
	}
	return true;
}

// Set the R_a RAM Table
bool BQ27441::setRaTable(uint16_t * ra)
{
	uint8_t raData[30];
	for (int i = 0; i < 15; i++) {
		raData[i*2]   = ra[i] >> 8;
		raData[i*2+1] = ra[i] & 0x00FF;
	}
	return writeExtendedData(BQ27441_ID_R_A_RAM, 0, raData, 30);
}
...
{
	uint8_t th;
	readExtendedData(BQ27441_ID_DISCHARGE, 0, &th, 1);
	return th;
}
...
{
	uint8_t th;
	readExtendedData(BQ27441_ID_DISCHARGE, 1, &th, 1);
	return th;
}
...
{
	uint8_t th;
	readExtendedData(BQ27441_ID_DISCHARGE, 2, &th, 1);
	return th;
}
...
{
	uint8_t th;
	readExtendedData(BQ27441_ID_DISCHARGE, 3, &th, 1);
	return th;
}
...
{
	uint8_t del;
	readExtendedData(BQ27441_ID_STATE, 26, &del, 1);
	return del;
}
...
// Issue a soft-reset to the BQ27441-G1A (Take OCV measurement and resimulation)
bool BQ27441::softReset(void)
{
	return executeControlWord(BQ27441_CONTROL_SOFT_RESET);
}

// Issue an exit-resim to the BQ27441-G1A (No OCV measurement, only resimulation)
bool BQ27441::exitResim(void)
{
	return executeControlWord(BQ27441_CONTROL_EXIT_RESIM);
}
...
// Read a specified number of bytes from extended data specifying a class ID and position offset
bool BQ27441::readExtendedData(uint8_t classID, uint8_t offset, uint8_t * data, uint8_t len)
{

...
const int hiberVoltage = 3550;   // (mV) Hibernate voltage (SoC~15%) -> reduce upload frequency
...
//
bool bq27441_InitParameters(void) 
{
  while ( !(lipo.status() & BQ27441_STATUS_INITCOMP) ) { delay(1); }
  lipo.enterConfig();
  bool success = lipo.setCapacity(lipoDesignCapacity,lipoDesignEnergy);
  success = success && lipo.setTaperRate(lipoTaperRate);
  success = success && lipo.setTerminateVoltage(underVoltage);  //(mV)
  if (lipoSavedQmax == -1) {
    ...
    success = success && lipo.setQmax(lipoSavedQmax);
    success = success && lipo.setRaTable(lipoSavedRaTable);
...
bool bq27441_ReadRaTable(uint16 * ra)
{
...
void uploadData(const char* server) 
{
    ...
    uint16 lipoQmax = bq27441_ReadQmax();
    bq27441_ReadRaTable(lipoSavedRaTable);
    ...
    for (int i = 0; i < 15; i++)
      thingStatus += String(lipoSavedRaTable[i])+",";
    ...
    for (int i = 0; i < 15; i++)
      Serial.print(String(lipoSavedRaTable[i])+",");
    #endif //DEBUG_BQ27441_IT
    Serial.println();
    #endif //BQ27441_FUEL_GAUGE
    #endif //DEBUG_ESP8266
